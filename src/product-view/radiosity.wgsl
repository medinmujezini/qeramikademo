...
fn photon() {
  ...
  for (var i = 0; i < (PhotonBounces+1); i++) {
    // Find the closest hit of the ray with the scene's quads/triangles.
    ...
    // Check if hit a triangle (furniture model)
    if ((hit.quad & kTriangleHitFlag) != 0u) {
      let tri_idx = hit.quad & ~kTriangleHitFlag;
      let tri_color = triangles[tri_idx].color;
      
      // Bounce off triangle
      ray.start = hit.pos + hit.normal * 1e-5;
      ray.dir = normalize(reflect(ray.dir, hit.normal) + rand_unit_sphere() * 0.75);
      
      // Photon color is multiplied by the triangle's color
      color *= tri_color * (1 - LightAbsorbtion);
      continue;
    }
